...
- Webhook destination URLs and authorization headers supplied by the host application.
...
4. Console output remains local; Telegram and webhook alerts cross their configured transport boundaries (HTTPS by default; loopback HTTP only by explicit opt-in).
...
| Notification storm delays requests            | Fixed queue capacity, pre-normalization overload admission, dropped overflow, per-notifier deadlines/circuits, and non-awaited framework adapters                                                            | A notifier can consume network resources outside the request path; operators should configure sensible thresholds        |
| Runtime telemetry leaks or adds load          | Watcher is disabled by default, numeric-only, one low-frequency unref'd timer, no network endpoint, and bounded threshold events                                                                             | Sampling still consumes small local CPU; benchmark enabled intervals in the host workload                                |
| Telegram/webhook credential exposure          | Credentials are accepted only at runtime; webhook URLs require HTTPS by default (loopback HTTP requires explicit opt-in), headers/envelopes/responses are bounded, and transport errors omit response bodies | A compromised host process or destination can still read its own credentials/data; webhook configuration is trusted code |
...
- Webhook destinations and headers must come from trusted application configuration, never request data.
- Runtime watcher output must contain aggregate numeric readings only; it must not collect hostnames,
  environment variables, request payloads, stacks, or secrets.
